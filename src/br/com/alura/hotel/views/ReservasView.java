...
import br.com.alura.hotel.utils.EscreverEmArquivo;
	...
	public ReservasView() {
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				final Integer valorPorDia = 45;
				SimpleDateFormat formatoDeData = new SimpleDateFormat("dd/MM/yyyy");
				NumberFormat formatoMoeda = NumberFormat.getCurrencyInstance();

				if(!((JTextField) txtDataS.getDateEditor().getUiComponent()).getText().isBlank()){
					Integer diaEntrada = Integer.parseInt(formatoDeData.format(txtDataE.getDate()).substring(0, 2)),
							diaSaida = Integer.parseInt(formatoDeData.format(txtDataS.getDate()).substring(0, 2)),
							mesEntrada = Integer.parseInt(formatoDeData.format(txtDataE.getDate()).substring(4, 5)),
							mesSaida = Integer.parseInt(formatoDeData.format(txtDataS.getDate()).substring(4, 5)),
							anoEntrada = Integer.parseInt(formatoDeData.format(txtDataE.getDate()).substring(7)),
							anoSaida = Integer.parseInt(formatoDeData.format(txtDataS.getDate()).substring(7));

					if(diaSaida <= diaEntrada || mesSaida < mesEntrada || anoSaida < anoEntrada) {
						txtDataE.setDate(null);
						txtDataS.setDate(null);
						JOptionPane.showMessageDialog(null, "A data de saída deve ser posterior à data de entrada.");
					} else {
						diaSaida += (mesSaida - mesEntrada) * 28;
						valorFinal = (long) (diaSaida - diaEntrada) * valorPorDia;
						txtValor.setText(
							formatoMoeda.format(
								(long) (diaSaida - diaEntrada) * valorPorDia
							)
						);
					}
				}
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
					...
					EscreverEmArquivo.escreverEmArquivo(
							reservaController.salvar(
									new java.sql.Date(txtDataE.getDate().getTime()),
									new java.sql.Date(txtDataS.getDate().getTime()),
									Double.valueOf(valorFinal),
